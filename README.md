...
* (2020/04/29) 4월 5차년도 베이스 코드 업데이트
* (2020/05/26) 5월 5차년도 2차 베이스 코드 업데이트
* (2020/06/29) 6월 5차년도 1차 정식 코드 파라미터 예제 파일 업로드
* (2020/09/22) 9월 TTS Timestep 값 수정
* (2020/11/14) pitch 에러 수정
* (2020/11/18) pitch range 값 수정
* (2020/12/28) 최종 코드 확인
* (2021/05/10) 최종 코드 스타일 및 README 수정

***

## System/SW Overview
![server_img](./img/server.png)
* 본 시스템은 감정 기반 표정/입 생성 모듈의 서버 부분에 해당하며, 발화 텍스트, 발화 타이밍 정보, 오디오, 성별, 헤어 모델, 여섯 가지 감정별 세기를 입력으로 받아 해당되는 3D 얼굴 애니메이션 시계열 데이터를 xml 형태로 반환하는 시스템입니다.
* 발화 타이밍 정보는 입력 오디오에 대한 TTS 모듈의 아웃풋 데이터를 사용할 수 있습니다.
* 출력 파일인 xml 데이터는 애니메이션 이름, 성별, 헤어 모델, 감정 강도, 시간별 얼굴 블랜드쉐입의 가중치 값으로 이루어져 있습니다.
* 아래 그림은 출력된 애니메이션 시계열 데이터(.xml)를 본 모듈의 클라이언트 부분에 해당되는 안드로이드 시스템을 통해 별도로 렌더링하였을 때 표현되는 예시 이미지입니다.

![anim_img](./img/animation.jpg)

* 얼굴 모델 변경시, app/speech.py 파일 내 viseme_blendshapes과 viseme_map의 블랜드쉐입명을 새로운 캐릭터에 해당되는 것으로 수정하시기 바랍니다.
* 자세한 사항은 아래 논문을 참고해주시기 바랍니다.

### Paper
[한국어 동시조음 모델에 기반한 스피치 애니메이션 생성](http://journal.cg-korea.org/archive/view_article?pid=jkcgs-26-3-49), KCGS 2020
